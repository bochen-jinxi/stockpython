...
,T5 AS (
	SELECT *
	FROM  T10  
	WHERE zuidagao/kaishigao-1<0.38 AND zuixiaodi/kaishidi-1<0.01
	AND  zuidashangyingxianfudu<8 AND zuidaxiayingxianfudu<4)	
	--SELECT * FROM T5	 		
,T590 AS (
...
,T599 AS (				
	...
	WHERE B.pctChg>0 AND A.shou<B.shou  AND A.di<B.di AND B.val<0 AND ((C.pctChg<0  AND B.shou>C.maxval AND B.kai<C.shou) OR (C.pctChg>0  AND B.shou>C.maxval AND B.kai<C.kai)))
	--SELECT * FROM T599 
