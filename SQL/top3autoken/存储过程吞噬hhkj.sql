...
IF OBJECT_ID('sphhkj', 'P') IS NOT NULL
    DROP PROCEDURE sphhkj;
GO

CREATE PROCEDURE dbo.sphhkj
(
...
--WHERE riqi>='2021-01-14' AND riqi<='2021-02-09' AND code='002645' 
    ...
    ,T403 AS (
        SELECT * 
        FROM T402
        WHERE RowID = 1 AND riqihao >= 18-2 AND val > 0.002
    )
    ...
    ,T499 AS (
        SELECT 
            ...
            T3.pctChg,
            T4.di AS kaishidi, T4.gao AS kaishigao,
            T4.code, T4.riqi AS kaishiriqi,
            T3.riqi,
            MAX(T3.riqi) OVER (PARTITION BY T3.code) AS jieshuriqi,
            T3.riqihao, T3.shitifudu, 
            T4.riqihao AS zuigaojiariqihao, T3.riqihao AS zuihouriqihao,
    ...
    ,T502 AS (
        ...
        FROM T401 
        INNER JOIN T501 ON T401.code = T501.code AND T401.riqi = T501.kaishiriqi
    )
    ,T503 AS (
