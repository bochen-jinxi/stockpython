	...
	BEGIN
		;WITH T AS (
					...
					,T6 AS ( 
					 ...
					 --SELECT * FROM T12
					 ,T13 AS (
					 SELECT A.gaoriqi, A.riqi AS mairiqi,B.riqi AS zhariqi,c.riqi AS zhuriqi,A.code,C.riqihao,C.shou FROM T12 AS A INNER JOIN T90 AS B ON A.code = B.code INNER JOIN dbo.T90 AS C ON B.code = C.code
					 WHERE A.riqihao+1=B.riqihao AND B.riqihao+1=C.riqihao AND A.shou<A.di*1.01 AND  B.kai>A.shou*1.01  AND B.kai*1.0000000009>B.shou
					 AND B.di*1.01>B.kai AND C.di>A.di AND C.di>B.di AND C.pctChg>0 AND C.shou>c.gao/1.01 AND c.di*1.01>c.kai
					 )
					 INSERT INTO dbo.T904 ([gaoriqi],[mairiqi],[zhariqi],[zhuriqi],[code],[shou])
					 SELECT  [gaoriqi],[mairiqi],[zhariqi],[zhuriqi],[code],[shou]
					 FROM T13 WHERE riqihao=@i
