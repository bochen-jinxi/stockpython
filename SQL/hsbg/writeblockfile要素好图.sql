...
SELECT  DISTINCT   CONCAT('exec master.dbo.xp_cmdshell ''echo '+CONVERT(varchar(5), kaishiriqi, 10)+'-'+CONVERT(varchar(5), jieshuriqi, 10)+'                                       '+CONVERT(varchar(5), kaishiriqi, 10)+'-'+CONVERT(varchar(5), jieshuriqi, 10)+'                                                         >>C:\zd_zsone\T0002\blocknew\blocknew.cfg"''',';')
  ...
  SELECT  DISTINCT   CONCAT('exec master.dbo.xp_cmdshell ''echo '+IIF(LEN(code)=5,'71#'+REPLACE(REPLACE(code,'sh.',1),'sz.',0),REPLACE(REPLACE(code,'sh.',1),'sz.',0))+'   >>C:\zd_zsone\T0002\blocknew\"'+ CONVERT(varchar(5), kaishiriqi, 10)+'-'+ CONVERT(varchar(5), jieshuriqi, 10)+'                                      .blk"''',';')
  ,code,
