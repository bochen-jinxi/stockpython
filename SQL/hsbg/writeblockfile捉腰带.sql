...
SELECT  DISTINCT   CONCAT('exec master.dbo.xp_cmdshell ''echo '+RIGHT(CONVERT(varchar(6),NAME),3)+RIGHT(CONVERT(varchar(5), kaishiriqi, 10),2)+'-'+CONVERT(varchar(5), jieshuriqi, 10)+'                                       '+RIGHT(CONVERT(varchar(6),NAME),3)+RIGHT(CONVERT(varchar(5), kaishiriqi, 10),2)+'-'+CONVERT(varchar(5), jieshuriqi, 10)+'                                                         >>C:\zd_zsone2\T0002\blocknew\blocknew.cfg"''',';')
  ...
  SELECT  DISTINCT   CONCAT('exec master.dbo.xp_cmdshell ''echo '+IIF(LEN(code)=5,'71#'+code,IIF(LEFT(code, 1) IN( '6','9'), '1'+code, '0'+code)) +'   >>C:\zd_zsone2\T0002\blocknew\"'+ RIGHT(CONVERT(varchar(6),NAME),3)+RIGHT(CONVERT(varchar(5), kaishiriqi, 10),2)+'-'+ CONVERT(varchar(5), jieshuriqi, 10)+'                                      .blk"''',';')
  ,code,
